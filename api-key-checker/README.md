...
- **网页仪表盘** — 基于 Streamlit 的可视化界面，侧边栏输入新 Key 后保存即自动检测，已保存的 Key 在侧边栏单独展示
- **单平台快速检测** — 每个平台卡片右侧有独立「检测」按钮，只检测该平台；无余额接口或未输入 Key 时按钮灰色禁用
...
> **余额查询说明**：
> - DeepSeek / SiliconFlow / OpenRouter / Kimi：有公开余额接口，可查具体金额
> - **智谱AI**：余额接口权限受限，能验证 Key 有效性，但部分账号无法获取具体余额
> - **阿里通义千问**：无余额查询接口，仅验证 Key 有效性
