...
def check_key(session, key):
    ...
    if status_code == 200 and isinstance(data, dict):
        ...
        if isinstance(balance_infos, list) and balance_infos:
            info = balance_infos[0] if isinstance(balance_infos[0], dict) else {}
            ...
            if total_balance > 0:
                return "positive", f"✅余额大于0 - 币种: {currency} | 总余额: {total_balance} | 赠金余额: {granted_balance} | 充值余额: {topped_up_balance}"
            return "zero", f"⭕余额为0 - 币种: {currency} | 总余额: {total_balance} | 赠金余额: {granted_balance} | 充值余额: {topped_up_balance}"
