...
PLATFORM_NAME = "阿里云 Qwen"
PLATFORM_PREFIX = "qwen"
...
def fetch_with_retry(session, key):
    ...
    for attempt in range(MAX_RETRIES + 1):
        try:
            response = session.post(CHAT_URL, headers=headers, json=payload, timeout=30)
...
def check_key(session, key):
    ...
    if status_code == 200 and isinstance(data, dict):
        ...
        if choices and isinstance(choices, list) and len(choices) > 0:
            return "positive", f"✅Key 有效（余额充足，可调用模型）"

    if status_code == 401:
        return "invalid", f"❌不可用 - Key 无效或已过期（401）"
    if status_code == 429:
        error_data = data.get("error") or {}
        msg = error_data.get("message", "请求频率过高或余额不足")
        return "zero", f"⭕余额不足或限流 - {msg}"
...
def build_report(total_count, lines, positive_keys, zero_keys, fail_keys, invalid_keys, sep):
    ...
    report_lines.extend(
        [
            ...
            f"🟡余额为0/仅Key有效的 API Key ({len(zero_keys)}):",
            sep.join(zero_keys),
...
def main():
    ...
    print("ℹ️阿里云 Qwen 无公开余额查询接口，将调用 qwen-turbo 模型验证 Key 是否可正常使用。")
    print("-" * 60)
    ...
    print(f"🔢总共: {len(keys)} | ✅有效: {len(positive_keys)} | ⭕余额不足/限流: {len(zero_keys)} | ❌不可用: {len(invalid_keys)} | ⚠️检测失败: {len(fail_keys)}")
    print("-" * 60)
    if len(positive_keys) > 0:
        print("🟢有效的 API Key:")
        print(",".join(positive_keys))
    if len(zero_keys) > 0:
        print("🟡余额不足/限流的 API Key:")
