...
def check_key(session, key):
    ...
    credit_data = data.get("data") if isinstance(data, dict) else None
    if status_code == 200 and isinstance(credit_data, dict):
        total = to_float(credit_data.get("total_credits"))
        used = to_float(credit_data.get("total_usage"))
        remain = total - used
        if remain > 0:
            return "positive", f"✅余额大于0 - 总共: {total} | 已用: {used} | 剩余: {remain}"
        return "zero", f"⭕余额为0 - 总共: {total} | 已用: {used} | 剩余: {remain}"
