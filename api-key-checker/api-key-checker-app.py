...
with st.sidebar:
    ...
    for name in all_platforms:
        cols = st.columns([0.4, 2.6, 1.2])
        with cols[0]:
            checked = st.checkbox("", value=select_all, key=f"plat_{name}", label_visibility="collapsed")
        with cols[1]:
            st.markdown(f"<div style='padding-top:2px;font-size:14px'>{name}</div>", unsafe_allow_html=True)
        with cols[2]:
            pf = PLATFORMS[name]
            btn_disabled = not pf.get("has_balance", True)
            if st.button("检测", key=f"sb_{name}", disabled=btn_disabled, use_container_width=True):
                st.session_state["single_check_platform"] = name
        if checked:
...
# ─── 单独平台检测 ───
if st.session_state.get("single_check_platform"):
    platform_name = st.session_state.pop("single_check_platform")
    if platform_name in PLATFORMS and all_keys:
        platform_config = PLATFORMS[platform_name]
        platform_results = []
        progress_bar = st.progress(0, text=f"正在检测 {platform_name}...")
        with requests.Session() as session:
            for idx, key in enumerate(all_keys):
                status_text, detail = check_single_key(session, key, platform_name, platform_config)
                platform_results.append((key, status_text, detail))
                progress_bar.progress((idx + 1) / len(all_keys), text=f"检测 {platform_name}: {idx+1}/{len(all_keys)}")
        progress_bar.empty()
