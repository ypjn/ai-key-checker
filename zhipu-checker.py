...
PLATFORM_PREFIX = "zhipu"
# 余额查询接口（部分账号可用，非公开 API）
API_URL = "https://open.bigmodel.cn/api/llm/balance"
# 回落：通过 chat 接口验证 Key 有效性
...
def fetch_with_retry(session, method, url, key, json_data=None):
...
def check_key(session, key):
    # 1️⃣ 先尝试余额查询接口
    response, net_error = fetch_with_retry(session, "GET", API_URL, key)
    if response is None:
    ...
    if status_code == 403:
        return "invalid", f"❌不可用 - 无权限（403）"

    # 余额接口返回了有效数据
    if status_code == 200 and isinstance(data, dict):
        success = data.get("success")
        biz_data = data.get("data") or data
        if isinstance(biz_data, dict) and success is not False:
            total_balance = to_float(biz_data.get("totalBalance") or biz_data.get("total_balance") or biz_data.get("availableBalance") or 0)
            used_balance = to_float(biz_data.get("usedBalance") or biz_data.get("used_balance") or 0)
            remaining_balance = to_float(biz_data.get("remainingBalance") or biz_data.get("remaining_balance") or biz_data.get("availableBalance") or 0)
            if remaining_balance == 0 and total_balance > 0:
                remaining_balance = total_balance - used_balance
            currency = str(biz_data.get("currency") or "CNY")
            if remaining_balance > 0:
                return "positive", f"✅余额大于0 - 币种: {currency} | 总余额: {total_balance} | 已用: {used_balance} | 剩余: {remaining_balance}"
            return "zero", f"⭕余额为0 - 币种: {currency} | 总余额: {total_balance} | 已用: {used_balance} | 剩余: {remaining_balance}"

    # 2️⃣ 余额接口不可用 → 通过 chat 调用验证 Key 有效性
    msg = ("ZhipuAI 未提供公开的余额查询 API，\n"
           "  将通过调用对话接口验证 API Key 可用性。")
    return _verify_via_chat(session, key, msg)


def _verify_via_chat(session, key, note=""):
    """通过轻量级 chat 调用验证 Key 是否有效"""
    payload = {
        "model": "glm-4-flash",
        "messages": [{"role": "user", "content": "hello"}],
        "max_tokens": 1,
    }
    response, net_error = fetch_with_retry(session, "POST", CHAT_URL, key, json_data=payload)
    if response is None:
        return "fail", f"⚠️检测失败 - 网络错误: {net_error}"
    ...
    if status_code == 200 and isinstance(data, dict):
        ...
        if choices and isinstance(choices, list) and len(choices) > 0:
            return "positive", f"✅Key 有效（余额充足，可调用模型） - {note}" if note else "✅Key 有效（余额充足，可调用模型）"
...
def build_report(total_count, lines, positive_keys, zero_keys, fail_keys, invalid_keys, sep):
    ...
    report_lines.extend(
        [
            f"🟢有效的 API Key ({len(positive_keys)}):",
...
def parse_args():
    parser = argparse.ArgumentParser(
        description=f"{PLATFORM_NAME} API Key 批量检测脚本\n注意: 该平台未提供公开的余额查询接口，将通过调用对话接口验证 Key 是否可正常使用（能调用说明余额充足）。"
    )
...
def main():
    ...
    print("ℹ️ZhipuAI 未提供公开余额查询接口，将通过调用对话接口验证 Key 是否可正常使用。")
    print("  (若欠费会返回 429，能调用模型说明余额充足)")
    print("-" * 60)
    ...
    print(f"🔢总共: {len(keys)} | ✅有效: {len(positive_keys)} | ⭕余额为0: {len(zero_keys)} | ❌不可用: {len(invalid_keys)} | ⚠️检测失败: {len(fail_keys)}")
    print("-" * 60)
